...
class CPRDoc():
    ...
    def __init__(self, doc, verbose=False):
        ...
        self.entity_list = list(self.entities.values())
        # need to sort entities from dict
        self.entity_list.sort(key=cmp_to_key(ent_cmp))
        # print(self.doc_id)
            ...
            for src, target in itertools.permutations(ent_list, 2):
                ...
                if target_rel is None:
                    ...
                    if fo is not None:
                        ...
                        print('\t'.join([str(s).lower() for s in out_list]))  # map(unicode.lower, out_list)))
                ...
                out_list = [str(s) for s in out_list]
                
                if fo is not None:
                    fo.write('\t'.join(out_list) + '\n')
                else:
                    # print('\t'.join(map(unicode.lower, out_list)))
                    print('\t'.join([s.lower() for s in out_list]))
